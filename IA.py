...
class IA:
    ...
    Methods
    -------
    beforeIaSetup(difficulty)
        Set up parameters and difficulty

    getColorTarget()
        Get and update the targetColorIA parameter

    displayChemin(listeChemin)
        Display the path of the AI clearly

    goUpIa(i, j, listPawnIabis, pawnID)
        Move the pawnIA up until an obstacle

    goLeftIa(i, j, listPawnIabis, pawnID)
        Move the pawnIA left until an obstacle

    goRightIa(i, j, listPawnIabis, pawnID)
        Move the pawnIA right until an obstacle

    goDownIa(i, j, listPawnIabis, pawnID)
        Move the pawnIA down until an obstacle

    IaBrutForce(limite, listPawnIa, pawn_color, limite_max, listeChemin)
        AI that test nearly all combinaisons until the limit is reached

    isUpIa(i, j, listPawnIa)
        Check if there is a Wall to access the up Case. Use the AI iterative grid.

    isLeftIa(i, j, listPawnIa)
        Check if there is a Wall to access the left Case. Use the AI iterative grid

    isRightIa(i, j, listPawnIa)
        Check if there is a Wall to access the right Case. Use the AI iterative grid

    isDownIa(i, j, listPawnIa)
        Check if there is a Wall to access the down Case. Use the AI iterative grid

    testDeuxPions(pion, pion2, limite, listPawnIa, limite_max, listeChemin)
        AI that test smartly all combinaisons with two pawns until limit is reached

    testFirstSolution(pion, pion2, limite, listPawnIa, limite_max, listeChemin)
        AI that test smartly all combinaisons with two pawns and stop at the first sucess

    testPionUnique(pion, limite, listPawnIa, limite_max, listeChemin)
        AI that test smartly all combinaisons with the main pawn until limit is reached
    """
    ...
    def beforeIaSetup(self, difficulty):
        # Pour setup l'ia, comme ça on évite des répétitions de boucles inutiles
